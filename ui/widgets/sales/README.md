...
- `sales_history_tab.py` : Historique des tickets de caisse, remboursements et clôtures de session avec affichage explicite de la caisse, montants d'ouverture/clôture, écarts, filtre par caisse et ouverture de la fiche détaillée par double-clic.
- `pos_payment_dialog.py` : Dialogue de règlement multi-moyens (espèces, carte, etc.).
- `dialogs.py` : Dialogues du point de vente (`ClientDialog`, `OpenSessionDialog`, `CloseSessionDialog`, `CashSessionDetailsDialog` affichant l'intégralité d'une session avec montants d'ouverture, sortie et ventes associées, `QuickCashPaymentDialog`, `SelectBatchBarcodeDialog` avec choix et filtrage de lieu de retrait, `EnterProductBarcodeDialog`).
