...
- **`_actions.py`**: Handles batch operations (FEFO validation, direct use, transfer, unpack & retail unit transfer, reclamation notes).
...
- **`_context_menu.py`**: Context menu for right-click actions on batch rows (quick consume, transfer, unpack to retail unit, batch details).
- **`_permissions.py`**: Role-based permissions enforcing visibility of financial and pricing columns (columns 2 through 8).
