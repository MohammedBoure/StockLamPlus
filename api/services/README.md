...
- **`inventory_count_service.py`** : Gestion complète des sessions de comptage d'inventaire physique mobile (création avec snapshot, listage, résumé chiffré/financier, consultation, scan individuel et groupé, modification de ligne, revue, annulation, suppression, détection des conflits de snapshot (`get_session_conflicts`) et application sécurisée des écarts au stock réel avec arbitrage `conflict_resolutions`).
