...
- **`_actions.py`**: Handles batch operations (FEFO validation, direct use, transfer, unpack & retail unit transfer, sales prices adjustment, price change history audit, reclamation notes).
...
- **`_context_menu.py`**: Context menu for right-click actions on batch rows (quick consume, transfer, unpack to retail unit, sales price adjustment, price change history, batch details).
